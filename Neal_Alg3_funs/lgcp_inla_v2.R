lgcp_inla_v2 = function(Y, Ns=1, T, mesh1, spde1, mlik1, config1, zgrid.eval=FALSE, tgrid, save_result=FALSE,verbose1=FALSE){
  # in the ..._v2 version, I corrected the E_point_processes by multiply Ns. In consequence, 
  # we don't need -log(N) in the estimation of lambda, and when sampling from lambda, the sample
  # is also correct now. 
  
  # Input:
  # Y: a vector containing the location of the events. 
  # Ns: the total number of samples that consistute Y. 
  ...
  # zhu: if using Ns to scale here, then we shall remove -log(Ns) in the log intensity estimation.
  ...
  result = inla(formula, data=data, family="poisson", control.predictor=list(A=ObservationMatrix),
                E=E_point_process, control.compute=list(mlik=mlik1, config=config1), verbose=verbose1)  
  ...
  if (mlik1==TRUE){
    mlike = result$mlik + Ns*(T[2]-T[1])  # here added the Ns*|C| to the log likelihood. 
  }else{ mlike = NULL}
